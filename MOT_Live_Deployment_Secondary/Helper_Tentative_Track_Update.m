...
function [tentative_tracks, tracks, next_track_ID, track_colours] = ...
    Helper_Tentative_Track_Update( ...
                    ...
                    Point_3d, meas_timestamp, is_camera_hit, P_birth_default, ...
                    IMM_TPM, base_colours, max_possible_velocity)
    
    %% A. Check if this 3D point matches an existing tentative track
    
    for tt = 1:length(tentative_tracks)
        distance = norm(tentative_tracks(tt).Pos_latest - Point_3d);
    
        if distance < 1 % metres (tune to usual bird displacement between hits)
            ...
            % Set camera hit flag if hit came from camera 
            if is_camera_hit 
                tentative_tracks(tt).has_camera_hit = true; 
            end 
    
            % Promote after 4 hits (ensure these four hits contain one
            % camera hit too, not just lidar hits) 
            if tentative_tracks(tt).hits >=  max_allowed_hits && tentative_tracks(tt).has_camera_hit
                ...
                if dt_vel > 1e-3
                    ...
                    if norm(V0) > max_possible_velocity
                     ...
                     % Reject birth position of a tentative track if behind camera (negative x)
                    if Point2(1) < 0.0
                        tentative_tracks(tt) = [];
                        return;
                    end
                    ...
                    % DEBUG 1: 
                    fprintf('BIRTH: Track %d at [%.2f, %.2f, %.2f] vel=[%.2f, %.2f, %.2f]\n', ...
                             new_track.id, new_track.X(1), new_track.X(2), new_track.X(3), ...
                             new_track.X(4), new_track.X(5), new_track.X(6));
    ...
    % Therefore, it is first point for a potentially newly detected object 
    ...
    % if cam triagulation creates new tt, only 3 more hits from any source to promote it 
    new_tentative.has_camera_hit   = is_camera_hit; 
