...
function Plot_Detection_Distribution_6(detection_stats)
    ...
    LIDAR_pct = sum(LIDAR_has_detections) / total_steps; 
    ...
    active_camera_steps = sum(cam1_has_detections | cam2_has_detections); 
    if active_camera_steps == 0
        dual_given_camera_pct = 0; 
        lidar_given_camera_pct = 0;
    else 
        lidar_given_camera_pct = 100 * sum(LIDAR_has_detections & active_camera_steps) / sum(active_camera_steps);
    ...
    if sum(LIDAR_has_detections) == 0
        ...
        camera_given_lidar_pct = 100 * sum(LIDAR_has_detections & active_camera_steps) / sum(LIDAR_has_detections);
    end
