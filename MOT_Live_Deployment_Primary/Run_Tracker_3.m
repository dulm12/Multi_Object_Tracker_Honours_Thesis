function [tracks, track_histories, track_colours, detection_stats] = Run_Tracker_3(measurement_log, LIDAR_log, ...
    ...
    % Obtain chi2 threshold from probability. 99% confidence gate, 2 because 2 DOF for (u,v)
    chi2_threshold = 2 * gammaincinv(chi2_probability, 2/2, 'lower'); 
    ...
    base_colours = [lines(7); hsv(30); jet(30); parula(30)];
    base_colours = base_colours(randperm(size(base_colours,1)), :);
    ...
    for k = 1:num_predicted_steps
        ...
        % After both passes and the update, handle the missed frames and
        % deletion 
        for i = 1:length(tracks) 
            if was_associated_cam1(i) || was_associated_cam2(i) 
        ...
        for i = 1:length(tracks)
            ...
            if track_id > numel(track_histories) || isempty(track_histories{track_id})
                track_histories{track_id} = [current_time, tracks(i).X', tracks(i).mu'];
    
            else % take existing history for Bird #5 (100 rows) and add curr. state (101st row at the bottom).
                track_histories{track_id} = [track_histories{track_id}; current_time, tracks(i).X', tracks(i).mu'];
            end
        end
